...
using Assets.Scripts.NetherSDK.Models;
...
namespace NetherSDK
{
    public class NetherClient : MonoBehaviour
    {
        ...
        public void Register(string deviceId, Action<CallbackResponse<Login>> callback)
        {
            Utilities.ValidateForNull(callback);
            StartCoroutine(CallNetherServiceInternal(
                new Login { providerId = deviceId }, 
                string.Format("{0}/api/registration/device", NetherDeploymentUrl), 
                HttpMethod.Post,
                _netherAuthorizationToken, 
                callback,
                containsResultProperty: false,
                requireToken: false));
        }
        ...
        private IEnumerator CallNetherServiceInternal<T>(T instanceToPostOrPut, string url, HttpMethod method, string authorizationToken,
            Action<CallbackResponse<T>> onCallNetherServiceCompleted, bool containsResultProperty = true, bool requireToken = true)
            where T : class
        {
            if (requireToken && string.IsNullOrEmpty(_netherAuthorizationToken))
            {
        ...
        public void GetNetherAccessToken(string login, string password, Action<string> callback)
        {
            Utilities.ValidateForNull(login, callback);
            Utilities.ValidateForNull(password, callback);
            StartCoroutine(GetAccessTokenByPassword(login, password, callback));
        }
        ...
        public void GetNetherAccessToken(string facebookUserAccessToken, Action<string> callback)
        {
            ...
            StartCoroutine(GetAccessTokenFromFbToken(facebookUserAccessToken, callback));
        }


        private IEnumerator GetAccessTokenByPassword(string login, string password, Action<string> callback)
        {
        ...
        private IEnumerator GetAccessTokenFromFbToken(string facebookUserAccessToken, Action<string> callback)
        {
            WWWForm form = new WWWForm();
            form.AddField("client_id", NetherClientID);
            form.AddField("client_secret", NetherClientSecret);
